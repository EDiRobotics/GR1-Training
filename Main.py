...
def train(acc, train_prefetcher, test_prefetcher, preprocessor, policy, env, eva, eval_dir, optimizer, scheduler, device, cfg, step, writer):
    '''
    prof = profile(
    ...
    for epoch in range(cfg['num_epochs']):
        if epoch % cfg['save_epochs'] == 0:
            ...
            if epoch != 0: 
                ...
                state_dict = {k: v for k, v in acc.unwrap_model(policy.net).state_dict().items() if not any(module_name in k for module_name in modules_to_exclude)}
                acc.save(state_dict, cfg['save_path']+'policy_{}.pth'.format(epoch+cfg['load_epoch']))
                policy.ema.copy_to(policy.ema_net.parameters())
                state_dict = {k: v for k, v in acc.unwrap_model(policy.ema_net).state_dict().items() if not any(module_name in k for module_name in modules_to_exclude)}
                acc.save(state_dict, cfg['save_path']+'ema_{}.pth'.format(epoch+cfg['load_epoch']))
        ...
        while batch is not None:
            with acc.accumulate(policy.net):
                ...
                loss = policy.compute_loss(
                    ...
                    skip_rgb_frame=cfg['skip_frame'],
                    actions=batch['actions'],
                    action_mask=batch['mask'],
                    action_loss_ratio=cfg['arm_loss_ratio'],
                )
...
if __name__ == '__main__':
    ...
    if os.path.isfile(cfg['save_path']+'step.json'):
        ...
        step = 0
    optimizer = torch.optim.AdamW(policy.net.parameters(), lr=cfg['lr_max'], weight_decay=cfg['weight_decay'], fused=True)
    ...
    # Train
    train(acc, train_prefetcher, test_prefetcher, preprocessor, policy, env, eva, eval_dir, optimizer, scheduler, device, cfg, step, writer)
